        ...
        try {
            ...
            if (response.isSuccess && response.result) {
                ...
                reset({
                    ...
                    gender: user.gender !== null ? user.gender : true, // Default to Male if null
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const submitData: Partial<User> = {
        ...
        gender: data.gender,
        birthday: format(data.birthday, "yyyy-MM-dd'T'HH:mm:ss"), // Keep ISO format or matches API expectation
        // Email is usually not editable via this endpoint or handled separately, but included if API allows
      };

      const response = await UserServices.editUserProfile(submitData);
