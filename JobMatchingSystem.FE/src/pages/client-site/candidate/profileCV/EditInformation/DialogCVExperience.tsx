...
import { CalendarIcon, X, Briefcase, ChevronDown } from "lucide-react";
...
interface DialogCVExperienceProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
  experienceToEdit?: CVExperience | null;
}

export function DialogCVExperience({
  open,
  onOpenChange,
  onSuccess,
  experienceToEdit,
}: DialogCVExperienceProps) {
    ...
    if (open) {
      if (experienceToEdit) {
        reset({
          ...
          endDate: new Date(experienceToEdit.endDate),
        });
      } else {
        reset({
          companyName: "",
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (experienceToEdit?.id) {
        await CVExperienceServices.update(experienceToEdit.id.toString(), submitData);
        toast.success("Cập nhật kinh nghiệm thành công");
      } else {
        await CVExperienceServices.create(submitData);
      ...
      console.error(error);
      toast.error(experienceToEdit ? "Có lỗi xảy ra khi cập nhật" : "Có lỗi xảy ra khi thêm mới");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in-0">
      ...
      >
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">
                {experienceToEdit ? "Cập nhật kinh nghiệm" : "Thêm kinh nghiệm"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {experienceToEdit 
                  ? "Cập nhật thông tin kinh nghiệm làm việc của bạn" 
                  : "Nhập thông tin kinh nghiệm làm việc của bạn"}
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="flex items-center justify-end gap-2 p-6 pt-4 border-t border-gray-100 shrink-0">
          ...
          >
            {actionLoading ? "Đang lưu..." : "Lưu kinh nghiệm"}
          </Button>
        </div>
      </div>
    </div>
